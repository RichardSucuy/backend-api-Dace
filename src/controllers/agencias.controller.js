...
const getAgencias = async (req, res) => {
    pool.query('SELECT * FROM agencias', async (err, result) => {
        if (err) {
            console.error('Error al obtener las agencias:', err.stack);
            res.status(500).json({ error: 'Error al buscar las agencias' });
        } else {
            const agencias = await Promise.all(
                result.rows.map(row => mapAgenciaRow(row))
            );
...
const queryAgenciaByID = async(id) => {
    const query = `select * from agencias where id_agencia = $1`;
    const values = [id];
    ...
    try {
        ...
        console.log( error );
        throw new Error('Error al buscar agencia por ID');
    }
}

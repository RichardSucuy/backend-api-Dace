...
const queryCiudadByID = async(id) => {
    ...
    try {
        ...
        console.log( error );
        throw new Error('Error al buscar la ciudad por ID');
    }
}
...
const getCiudadesByID = async(req, res) => {
    ...
    try {
        const ciudades = await queryCiudadByID(id);
        ...
        res.status(500).json({ error: 'Error al buscar la ciudad por ID' });
    }   
};
